...
We have all this information from the table generated by checkm, and we can use it to extract information. Now we have 9 MAGs and we can easily pick manually those MAGs for quality. Let's say > 70 % completeness and < 5 % contamination by looking into this table. But what if we had ended up with hundred or thousan of genomes ? Doing a manual selection would not be a easy option. So now let's code a little script to automatically select those *"Good quality MAGs"*:
